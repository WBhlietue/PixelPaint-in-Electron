...
document.getElementById("colorHex").oninput = ()=>{
    const result = (/^#[0-9A-F]{6}$/i.test(document.getElementById("colorHex").value))
    if(result){
        document.getElementById("colorPanel").value = document.getElementById("colorHex").value
    }
}
...
function CreatePage() {
    ...
    function InitPanelEvent() {
        ...
        paintWindowConfig.scaleInit =
            main.offsetWidth / paintWindow.offsetWidth;
        ...
        function Scroll(e) {
            if (e.ctrlKey) {
                if (e.deltaY > 0) {
                    paintWindowConfig.scale -=
                        0.1 * paintWindowConfig.scaleInit;
                } else {
                    paintWindowConfig.scale +=
                        0.1 * paintWindowConfig.scaleInit;
                }
        ...
        PaintWindowSet();
        const size = parseInt(
            paintWindow.offsetHeight / paintWindowConfig.height
        );
